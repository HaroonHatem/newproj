...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($r->num_rows > 0) $error = 'البريد مستخدم سابقاً';
    ...
    $stmt2 = $conn->prepare('INSERT INTO users (name,email,password,user_type,phone,website) VALUES (?,?,?,?,?,?)');
    $stmt2->bind_param('ssssss', $name, $email, $hash, $user_type, $phone, $website);
    if ($stmt2->execute()) {
  ...
  <main class="container">
    <div class="card form-card">
      ...
      <form method="post">
        ...
        <div class="form-grid"><input class="input" name="password" type="password" placeholder="كلمة المرور" required><input class="input" name="phone" placeholder="هاتف الشركة"></div><input class="input" name="website" placeholder="موقع الشركة (اختياري)"><button class="btn btn-primary" type="submit">إنشاء حساب شركة</button>
      </form>
    </div>
  </main>
</body>
